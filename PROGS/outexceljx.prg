...
if !empty(cTemplate)
	...
	FldCnt1 = xlsheet.cells(1,3).value &&field's count
...
RangeString = xlsheet.cells(RecCnt+StartRow-1,FldCnt1).address
RangeString ="$A$5:"+ RangeString
...
*!*	RangeString = "$A$5:"+xlsheet.cells(2,FldCnt).address
...
*!*	RangeString = "$A$5:"+xlsheet.cells(RecCnt+StartRow-1,1).address
*!*	xlSheet.range(RangeString).borders(1).LineStyle = 7&&12
*!*	RangeString = xlsheet.cells(5,FldCnt).address+":"+xlsheet.cells(RecCnt+StartRow-1,FldCnt).address
...
xlSheet.cells(2,1).value = "单位:"+ALLTRIM(dwmc(m.pdwbm))+"（"+ALLTRIM(m.pdwbm)+"）"
