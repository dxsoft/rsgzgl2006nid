...
Add-Check -Rows $rows -Name "workbench-salary-grade-todo-entry" -Passed ($indexHtml -match 'id="generateSalaryGradeTodoButton"[\s\S]*?&#29983;&#25104;&#34218;&#32423;&#24453;&#21150;' -and $appJs -match 'generateSalaryGradeTodo' -and $appJs -match '/api/workbench/salary-grade-applications/generate' -and $appJs -match 'salaryGradeCount') -Message "Workbench should expose a dedicated normal salary grade todo generator for institution staff."
...
Add-Check -Rows $rows -Name "workbench-app-script-cache-busted" -Passed ($indexHtml -match '<script src="/app\.js\?v=[^"]+"></script>') -Message "Static app script should be cache-busted so repaired workbench handlers are loaded by the browser."
Add-Check -Rows $rows -Name "workbench-assessment-batch-entry" -Passed ($appJs -match 'SALARY_ASSESSMENT_BATCH' -and $appJs -match 'openAssessmentBatch' -and $appJs.Contains('const ASSESSMENT_BATCH_CANDIDATES_ENDPOINT = "/api/persons/assessments/batch-candidates";') -and $appJs.Contains('const ASSESSMENT_BATCH_SAVE_ENDPOINT = "/api/persons/assessments/batch";') -and $appJs.Contains('Api.request(`${ASSESSMENT_BATCH_CANDIDATES_ENDPOINT}?') -and $appJs -match 'Api\.request\(ASSESSMENT_BATCH_SAVE_ENDPOINT,\s*\{[\s\S]*?method:\s*"POST"') -Message "Workbench should expose annual assessment batch as a top-level menu entry, load candidates through the GET candidates endpoint, and save through POST only."
Add-Check -Rows $rows -Name "workbench-assessment-batch-feedback" -Passed ($appJs.Contains("\u5e74\u5ea6\u8003\u6838\u4eba\u5458\u540d\u5355") -and $appJs.Contains("\u52a0\u8f7d\u5931\u8d25")) -Message "Assessment batch entry should show immediate loading and error feedback in the result panel."
